...
res=1
echo "Waiting for docker-swarm token..."
while [ $res -ne 0 ]; do 
    sleep 1
...
echo "Fetching docker-swarm token..."
scp -q centos@$1:~/swarm-join.txt .

echo "Booting workers..."
nova boot --min-count 3 --max-count 3 --flavor m1.small --image FC28atomic --nic net-id=c1445469-4640-4c5a-ad86-9c0cb6650cca --security-group default --meta swarm-join="$(cat swarm-join.txt)" --user-data fc-atomic.sh --key-name mykey FC28atomic_${OS_USERNAME}
echo "Done!"
